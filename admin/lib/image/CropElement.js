...
function CropElement(container, param, observer) {
  ...
  window.addEventListener('resize',
    function() {
      if (!this._canvas || !this._param.crop) return;
      // XXX copypaste
  ...
  function onMouseMove(evt) {
    var _evt = evt;
    // console.log(this._center.clientWidth);
    if (evt.touches) evt = evt.touches[0];
    if (this._canvas && this._x1 && this._param.crop) {
      ...
      if ((left + this._canvas.width * this._canvasScale) < this._center.clientWidth) {
        left = this._center.clientWidth - Math.floor(this._canvas.width * this._canvasScale);
      }
      if ((top + this._canvas.height * this._canvasScale) < this._center.clientHeight) {
        top = this._center.clientHeight - Math.floor(this._canvas.height * this._canvasScale);
      }
  ...
  this.get = function() {
    ...
    ctx.fillStyle = this._param.fillColor || '#fff';
    ctx.fillRect(0, 0, c.width, c.height);
    if (this._param.crop) {
      // "Math.floor" is using for fix bug in Safari with "white pic"
      ctx.drawImage(this._canvas,
          Math.floor(-this._canvasShiftX), Math.floor(-this._canvasShiftY), Math.floor(this._center.clientWidth / this._canvasScale), Math.floor(this._center.clientHeight / this._canvasScale),
          0, 0, c.width, c.height);

    } else {
        ctx.drawImage(this._canvas,
          (1 - this._canvas.getBoundingClientRect().width / this._center.getBoundingClientRect().width) * c.width / 2, (1 - this._canvas.getBoundingClientRect().height / this._center.getBoundingClientRect().height) * c.height / 2,
          c.width * this._canvas.getBoundingClientRect().width / this._center.getBoundingClientRect().width, c.height * this._canvas.getBoundingClientRect().height / this._center.getBoundingClientRect().height);
    }
  ...
  this.set = function(canvas) {
    ...
    if (canvas) {
      ...
      if (this._param.crop) {
        ...
        if (scaleX > scaleY) {
          this._canvas.style.height = this._center.clientHeight + 'px';
          this._canvas.style.width = 'auto';
          this._canvas.style.left = (this._center.clientWidth - this._canvas.getBoundingClientRect().width) / 2 + 'px';
        } else {
          this._canvas.style.width = this._center.clientWidth + 'px';
          this._canvas.style.height = 'auto';
          this._canvas.style.top = (this._center.clientHeight - this._canvas.getBoundingClientRect().height) / 2 + 'px';
        }
      }
    }
